<#
This script registers multiple DNS names for a single server by getting it's IP Address and connecting to a Windows DNS Server and registering all the names in the array DNSNames
...
Invoke-Command -computername $DNSServer -Credential $Credential -argumentlist ($DNSServer, $IPAddr, $DNSNames) -scriptblock {
	...
	for ($i = 0; $i -lt $DNSNames.Count; $i++) {
		...
		Add-DnsServerResourceRecord -ComputerName $DNSServer -ZoneName $zone -A -Name $name -IPv4Address $IPAddr -TimeToLive 0:0:5:0
	}
